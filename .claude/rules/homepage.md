...
- **GitHub 푸시(동기화)는 사용자가 "올려", "동기화", "푸시해" 라고 명시적으로 요청할 때만 한다.** 자동으로 push 금지.
...
- **사용자가 요청하지 않아도 수정 후 반드시 자체 테스트를 실행한다.** 로컬 서버를 띄우고 Playwright로 스크린샷을 찍어 직접 확인한다. 특히:
  - 폰트 대비 (배경색 대비 텍스트가 잘 보이는지)
  - 드롭다운/팝업이 정상 열리고 보이는지 (overflow 잘림 없는지)
  - JS 에러 없는지 (pageerror 리스너)
  - 데스크탑/모바일 양쪽 모두 확인
