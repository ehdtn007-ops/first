...
- 계정 정보: .env 파일 참조 (ehdtn007@gmail.com, skaeh0404$)
...
3. **`git diff --cached` 로 확인** — 코드 내부에 하드코딩된 비밀번호/키가 있는지 확인 (ehdtn007@gmail.com/skaeh0404$같은 하드코딩도 주의).
4. **`git add .` / `git add -A` 사용 금지** — 반드시 파일명을 명시하여 `git add index.html admin.html` 식으로 선택적 스테이징.
5. **수정 후 컴퓨터 화면 매물지도 버전 및 게시판 버전, 휴대폰 화면 매물지도 버전 및 게시판 버전 가로, 세로. 모두 다 정상 작동하는지 확인.
6. 위 중 하나라도 빠지면 push 금지.
