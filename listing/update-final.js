...
// 최종 - API 테스트 통과한 확실한 지번
const FINAL = {
  'b20': { prov:'서울', area:'동대문구', dong:'답십리동', jibun:'292-1', floor:'2-3' },
  'b25': { prov:'서울', area:'은평구', dong:'수색동', jibun:'120', floor:'2-4' },
  'b26': { prov:'서울', area:'도봉구', dong:'도봉동', jibun:'20', floor:'2-3' },
  'b35': { prov:'서울', area:'중랑구', dong:'상봉동', jibun:'128', floor:'2-6' },
  'b37': { prov:'서울', area:'구로구', dong:'신도림동', jibun:'100', floor:'2-3' },
};
...
async function main() {
  ...
  console.log('최종 업데이트: ' + Object.keys(FINAL).length + '건\n');
  ...
  for (let i = 0; i < lids.length; i++) {
    ...
    try {
      ...
      if (bldg) {
        ...
        console.log('OK (층수:' + (bldg.totalfloor||'-') + ' 면적:' + (bldg.area2||'-') + ' 준공:' + (bldg.year||'-') + ' 주차:' + (bldg.parking||0) + ')');
        success++;
      } else {
